...
exports.handler = async (event, context) => {
  ...
  if (!WEATHER_KEY || !DUST_KEY || !KAKAO_KEY) {
    return {
      statusCode: 500,
      body: JSON.stringify({
        error: "Missing API Keys in Netlify Environment Variables",
      }),
    };
  }
  ...
  } else if (path.includes("/api/ultra-srt-fcst")) {
    targetBaseUrl =
  ...
  if (
    ...
    path.includes("/api/ultra-srt-fcst") ||
    path.includes("/api/forecast")
  ) {
    ...
    path.includes("/api/dust") ||
    path.includes("/api/sido-dust") ||
    path.includes("/api/tm-coord") ||
    path.includes("/api/nearby-station")
  ) {
  ...
  try {
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({
        error: "Netlify Proxy Request Failed",
