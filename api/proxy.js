export default async function handler(req, res) {
  ...
  if (pathname.includes("/api/weather")) {
  ...
  } else if (pathname.includes("/api/ultra-srt-fcst")) {
    targetBaseUrl =
    ...
    serviceKey = WEATHER_KEY;
  } else if (pathname.includes("/api/mid-land")) {
    // 중기육상
    targetBaseUrl =
      "https://apis.data.go.kr/1360000/MidFcstInfoService/getMidLandFcst";
    serviceKey = WEATHER_KEY;
  } else if (pathname.includes("/api/mid-ta")) {
    // 중기기온
    targetBaseUrl =
      "https://apis.data.go.kr/1360000/MidFcstInfoService/getMidTa";
    serviceKey = WEATHER_KEY;
  } else if (pathname.includes("/api/tm-coord")) {
    // [개선] 미세먼지 사각지대 해소를 위한 TM 좌표 변환
    targetBaseUrl =
      "https://apis.data.go.kr/B552584/MsrstnInfoInqireSvc/getTMStdrCrdnt";
    serviceKey = DUST_KEY;
  } else if (pathname.includes("/api/nearby-station")) {
    // [개선] 좌표 기반 근처 측정소 목록 조회
    targetBaseUrl =
      "https://apis.data.go.kr/B552584/MsrstnInfoInqireSvc/getNearbyMsrstnList";
    serviceKey = DUST_KEY;
  } else if (pathname.endsWith("/api/sido-dust")) {
    // 시도별 실시간 측정정보 조회
    targetBaseUrl =
      "https://apis.data.go.kr/B552584/ArpltnInforInqireSvc/getCtprvnRltmMesureDnsty";
    serviceKey = DUST_KEY;
  } else if (
    pathname.endsWith("/api/dust") ||
    pathname.endsWith("/api/dust-proxy")
  ) {
    // 측정소별 실시간 측정정보 조회
    targetBaseUrl =
    ...
    return res
      .status(404)
      .json({ error: "Endpoint Not Supported", path: pathname });
  }
  ...
  if (
    pathname.includes("/api/weather") ||
    pathname.includes("/api/ultra-srt-fcst") ||
    pathname.includes("/api/forecast")
  ) {
    ...
    pathname.includes("/api/dust") ||
    pathname.includes("/api/sido-dust") ||
    pathname.includes("/api/tm-coord") ||
    pathname.includes("/api/nearby-station")
  ) {
    if (!params.has("numOfRows")) {
      // 경기도 전체(약 120개 이상) 및 대도시 데이터 누락 방지를 위해 1000으로 설정
      const rows = pathname.includes("sido-dust") ? "1000" : "10";
